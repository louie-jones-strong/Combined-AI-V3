...
class Simulation(SimBase.SimBase):
	...
	def __init__(self):
		...
		self.BackGroundpieceList += [Piece.PieceBase([350,300],[150, 1], Shape.HorizontalLine(), [0, 0, 0])]
		self.BackGroundpieceList += [Piece.PieceBase([350,400],[150, 1], Shape.HorizontalLine(), [0, 0, 0])]
		
		self.BackGroundpieceList += [Piece.PieceBase([300, 350], [1, 150], Shape.VerticalLine(), [0, 0, 0])]
		self.BackGroundpieceList += [Piece.PieceBase([400, 350], [1, 150], Shape.VerticalLine(), [0, 0, 0])]
		return
	...
	def ComplexBoardOutput(self, board):
		...
		for x in range(grid[0]):
			for y in range(grid[1]):
				...
				if board[loop] != 0:
					if board[loop] == 1:#X
						pieceList += [Piece.PieceBase([((x+0.5)-grid[0]/2)*gridSize*2+350, ((y+0.5)-grid[1]/2)* gridSize*2+350], [pieceSize, pieceSize], Shape.Cross(), [0, 0, 0])]
					else:#O
						pieceList += [Piece.PieceBase([((x+0.5)-grid[0]/2)*gridSize*2+350, ((y+0.5)-grid[1]/2)* gridSize*2+350], [pieceSize, pieceSize], Shape.Circle(), [0, 0, 0])]
				loop += 1
		return pieceList
